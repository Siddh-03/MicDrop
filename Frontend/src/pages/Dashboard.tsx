...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sessionsResponse = await fetch("http://localhost:3000/api/sessions", { credentials: "include" });
  ...
  const handleUpdateSession = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/sessions/${editingSession._id}`, {
        method: 'PUT',
      ...
      setSessions(sessions.map(s => s._id === editingSession._id ? updatedSessionData : s));
        ...
        <main className="max-w-6xl mx-auto p-6">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              Manage your speaking sessions and track your presentation improvements.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card className="bg-gradient-card border shadow-card">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">All time presentations</p>
              </CardContent>
            </Card>
            <Card className="bg-gradient-card border shadow-card">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Link to="/create-session">
                <Card className="bg-gradient-primary text-primary-foreground hover:shadow-glow transition-all duration-300 hover:scale-105 cursor-pointer">
            ...
            <div className="space-y-4">
              {sessions.length > 0 ? (
                ...
                <Card className="bg-gradient-card border shadow-card text-center">
                  <CardContent className="pt-6">
                    <p className="text-muted-foreground">You have no recent sessions.</p>
                    <Link to="/create-session">
                      <Button variant="link" className="mt-2">Create one now</Button>
                    </Link>
                  </CardContent>
                </Card>
              )}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Make changes to your upcoming session.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="title" className="text-right">Title</Label>
              <Input id="title" value={updatedTitle} onChange={(e) => setUpdatedTitle(e.target.value)} className="col-span-3" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="gracePeriod" className="text-right">Grace Period</Label>
              <Select value={updatedGracePeriod} onValueChange={setUpdatedGracePeriod}>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={
                !updatedTitle.trim() || !updatedDate || !updatedTime
              }
            >Save Changes</Button>
          </DialogFooter>
        </DialogContent>
